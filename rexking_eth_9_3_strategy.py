...
class RexKingETH93Strategy:
    def __init__(self, capital=10000):
        ...
        self.risk_per_trade = 0.003  # 0.3%风险（提升）
        ...
        self.trailing_stop_multiplier = 2.5  # 移动止损2.5×ATR（放宽）
    ...
    def calculate_indicators(self, df):
        """计算技术指标 - 9.3优化"""
        print("Calculating indicators for RexKing ETH 9.3...")
    ...
    def mock_etherscan_data(self, df):
        """模拟Etherscan数据 - 9.3修复time_window"""
    ...
    def backtest(self, df):
        """回测策略 - 9.3修复TP/SL逻辑"""
        print("Starting RexKing ETH 9.3 backtest...")
        ...
        for i in range(50, len(df)):
            ...
            if (signal_strength > self.signal_strength_threshold and 
                ...
                stop_loss = atr * 0.8 * current_row['close']  # 价格单位
                
                if stop_loss <= 0:
                    continue
                
                position_size = risk_cap / stop_loss
                position_size = min(position_size, 0.1)  # 最大10%仓位
                
                # 入场价格
                entry_price = current_row['close'] * (1 + self.slippage_rate)
                
                # 止盈止损设置 - 9.3修复ATR单位
                tp1_price = entry_price * (1 + self.tp1_multiplier * atr)  # 修复：价格单位
                tp2_price = entry_price * (1 + self.tp2_multiplier * atr)  # 修复：价格单位
                trailing_sl_price = entry_price * (1 - self.trailing_stop_start * atr)  # 修复：价格单位
                
                print(f"  Entry: ${entry_price:.2f}, TP1: ${tp1_price:.2f}, TP2: ${tp2_price:.2f}, SL: ${trailing_sl_price:.2f}, Size: {position_size:.4f}")
            ...
            for pos in self.positions[:]:
                ...
                # 从0.2×ATR开始移动止损 - 9.3修复
                if current_price > pos['entry_price'] * (1 + self.trailing_stop_start * pos['atr']):
                    pos['trailing_sl_price'] = max(
                        pos['trailing_sl_price'], 
                        pos['max_price'] * (1 - self.trailing_stop_multiplier * pos['atr'])  # 修复：价格单位
                    )
                ...
                if not pos['partial_closed'] and current_price >= pos['tp1_price']:
                    ...
                    print(f"Trade closed: PNL=${net_profit:.2f}, Type=tp1, Size={partial_size:.4f}, Exit=${pos['tp1_price']:.2f}")
                ...
                # 第二目标止盈 - 9.3修复
                    ...
                    print(f"Trade closed: PNL=${net_profit:.2f}, Type=tp2, Size={pos['position_size']:.4f}, Exit=${pos['tp2_price']:.2f}")
                ...
                # 移动止损或超时平仓 - 9.3修复
                    ...
                    print(f"Trade closed: PNL=${net_pnl:.2f}, Type={exit_type}, Size={pos['position_size']:.4f}, Exit=${exit_price:.2f}")
    ...
    def diagnostics(self, df):
        """诊断各过滤器的通过率"""
        print("\n" + "="*50)
        print("REXKING ETH 9.3 DIAGNOSTICS")
        print("="*50)
    ...
    def analyze_results(self):
        ...
        print("\n" + "="*60)
        print("RexKing ETH 9.3 Strategy Results")
        print("="*60)
...
def main():
    # 初始化策略
    strategy = RexKingETH93Strategy(capital=10000)
    ...
    # 分析结果
    trades_df_april = strategy.analyze_results()
    
    # 保存交易记录
    if trades_df_april is not None and len(trades_df_april) > 0:
        trades_df_april.to_csv('rexking_eth_9_3_trades_2025_04.csv', index=False)
        print(f"\nApril trades saved to rexking_eth_9_3_trades_2025_04.csv")
    ...
    # 分析结果
    trades_df_may = strategy.analyze_results()
    
    # 保存交易记录
    if trades_df_may is not None and len(trades_df_may) > 0:
        trades_df_may.to_csv('rexking_eth_9_3_trades_2025_05.csv', index=False)
        print(f"\nMay trades saved to rexking_eth_9_3_trades_2025_05.csv")
