...
def generate_signals(df):
    ...
    cond_4h = (
        ...
        (df['ema20'] > df['ema60']) &
        (df['atr'] > 0.003 * df['close']) &  # ATR > 0.3%
        (df['bb'] > 2)  # BB > 2%
    )
    
    # 1H funding
    cond_1h = (df['funding'] < 0.00005)  # funding < 0.005%
    
    # 15m突破+量（优化条件：15%的4H均值）
    df['volume_4h_ma'] = df['volume'].rolling(20).mean()
    cond_15m = df['breakout_15m'] & (df['volmean_15m'] > 0.15 * df['volume_4h_ma'])
...
def main():
    ...
    if trades:
        ...
        trades_df.to_csv('rexking_eth_10_2_trades.csv', index=False)
        print(f"交易结果已保存到: rexking_eth_10_2_trades.csv")
