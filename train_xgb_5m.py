#!/usr/bin/env python3
"""
RexKing – XGBoost Training 5m (Walk-Forward)

读取 5m 特征数据, 使用walk-forward验证训练XGBoost模型, 输出AUC和SHAP重要性.
"""
...
def create_walk_forward_splits(df: pd.DataFrame, train_days: int = 180, test_days: int = 30, offset_days: int = 30):
    ...
    while current_start < end_date:
        ...
        # 确保有足够的样本
        if len(train_indices) >= 3000 and len(test_indices) >= 600:
            splits.append({
...
def analyze_walk_forward_results(results: list):
    ...
    # 保存结果
    out_csv = DATA_DIR / "walk_forward_5m_results.csv"
