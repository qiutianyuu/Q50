...
def scan_parameters(model, df, timeframe):
    """扫描参数空间"""
    print(f"🔍 开始参数扫描: {timeframe}")
    
    # 参数网格
    thresholds = [0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8]
    confidence_thresholds = [0.6, 0.7, 0.8, 0.85, 0.9]
    holding_periods = [2, 4, 6, 8, 12, 16, 24]  # 小时
    transaction_costs = [0.0005, 0.001, 0.0015, 0.002]  # 0.05% - 0.2%
...
def find_optimal_parameters(results_df):
    ...
    # 过滤掉交易次数太少的组合
    filtered_df = results_df[results_df['total_trades'] >= 10].copy()
    ...
    return {
        'best_by_returns': best_by_returns,
...
def main():
    ...
    results_15m = scan_parameters(model_15m, features_15m, '15m')
    results_15m.to_csv('param_scan_results_15m.csv', index=False)
    ...
    results_5m = scan_parameters(model_5m, features_5m, '5m')
    results_5m.to_csv('param_scan_results_5m.csv', index=False)
    ...
    print("\n✅ 参数扫描完成！")
    print("📁 结果文件:")
    print("  - param_scan_results_15m.csv")
    print("  - param_scan_results_5m.csv")
