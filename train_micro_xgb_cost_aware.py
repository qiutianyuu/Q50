...
def load_latest_labeled_data():
    """Load the latest cost-aware labeled features file"""
    files = glob.glob("data/micro_features_cost_aware_labels_*.parquet")
    if not files:
        raise FileNotFoundError("No cost-aware labeled features files found")
...
def main():
    ...
    # Save model
    model_file = f"xgb_micro_cost_aware_{timestamp}.bin"
    joblib.dump(model, model_file)
    print(f"\nModel saved: {model_file}")
    
    # Save feature importance
    importance_file = f"feature_importance_cost_aware_{timestamp}.csv"
    ...
    results_file = f"micro_model_results_cost_aware_{timestamp}.json"
